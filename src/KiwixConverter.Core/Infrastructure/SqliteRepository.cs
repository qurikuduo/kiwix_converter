...
";
        ...
        while (await reader.ReadAsync(cancellationToken))
        {
            ...
            switch (key)
            {
                ...
                case "defaultOutputDirectory":
                    ...
                    break;
                case "mainWindowLeft":
                    if (int.TryParse(value, out var mainWindowLeft))
                    {
                        settings.MainWindowLeft = mainWindowLeft;
                    }

                    break;
                case "mainWindowTop":
                    if (int.TryParse(value, out var mainWindowTop))
                    {
                        settings.MainWindowTop = mainWindowTop;
                    }

                    break;
                case "mainWindowWidth":
                    if (int.TryParse(value, out var mainWindowWidth))
                    {
                ...
                case "mainWindowHeight":
                    if (int.TryParse(value, out var mainWindowHeight))
                    {
                ...
                case "mainWindowState":
                    settings.MainWindowState = value;
                    break;
                case "rootSplitterDistance":
                    if (int.TryParse(value, out var rootSplitterDistance))
                    {
                ...
                case "weKnoraSyncUpperSplitterDistance":
                    if (int.TryParse(value, out var weKnoraSyncUpperSplitterDistance))
                    {
    ...
    public async Task SaveSettingsAsync(AppSettings settings, CancellationToken cancellationToken = default)
    {
        ...
        await UpsertSettingAsync(connection, transaction, "mainWindowLeft", settings.MainWindowLeft?.ToString(CultureInfo.InvariantCulture), cancellationToken);
        await UpsertSettingAsync(connection, transaction, "mainWindowTop", settings.MainWindowTop?.ToString(CultureInfo.InvariantCulture), cancellationToken);
        ...
        await UpsertSettingAsync(connection, transaction, "mainWindowState", settings.MainWindowState, cancellationToken);
